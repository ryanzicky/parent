...
public class Main71 {
    ...
    public static void main(String[] args) {
        ...
        try {
            highs = Arrays.stream(in.nextLine().split(" "))
        ...
        for (int i = 0; i < highs.size() - 1; i++) {
            if (i % 2 == 0 && highs.get(i) < highs.get(i + 1)) {
                swap(highs, i, i + 1);
            }
            if (i % 2 != 0 && highs.get(i) > highs.get(i + 1)) {
                swap(highs, i, i + 1);
        ...
        highs.forEach(x -> builder.append(x).append(" "));
    ...
    private static void swap(List<Integer> highs, int i, int i1) {
        int tmp = highs.get(i);
        highs.set(i, highs.get(i1));
        highs.set(i1, tmp);
    }
}
