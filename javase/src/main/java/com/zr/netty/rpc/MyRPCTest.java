    ...
    public void get() {
        ...
        for (int i = 0; i < threads.length; i++) {
            threads[i] = new Thread(() -> {
                Car car = proxyGet(Car.class); // 动态代理 实现
                car.ooxx("hello");
            });
        }
...
//        Fly fly = proxyGet(Fly.class);// 动态代理 实现
//        fly.ooxx("hello");
    }
...
class ServerRequestHandler extends ChannelInboundHandlerAdapter {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        if (buf.readableBytes() >= 110) {
            ...
            System.out.println("server response header.getRequestID(): " + header.getRequestID());
...
class ClientResponse extends ChannelInboundHandlerAdapter {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        if (buf.readableBytes() >= 110) {
            ...
            ObjectInputStream oin = new ObjectInputStream(in);
            MyHeader header = (MyHeader) oin.readObject();
            System.out.println("client response header.getRequestID(): " + header.getRequestID());
